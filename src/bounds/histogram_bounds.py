...
class HistogramBoundsExtractor(BoundsExtractor):
    ...
    def get_bounds(self, doc: pymupdf.Document, dpi: int | None) -> list[pymupdf.Rect]:
        ...
        for i in tqdm(range(doc.page_count)):
            ...
            rectangles.append(self._extract_page_rect(page, dpi))
        return rectangles

    def _get_bounds_threaded(
        self,
        doc: pymupdf.Document,
        dpi: int,
    ) -> list[pymupdf.Rect] | None:
        doc_path = doc.name
        if not doc_path:
            return None
        ...
        max_workers = min(page_count, cpu_count, self._MAX_THREAD_WORKERS)
        if max_workers <= 1:
            return None

        rectangles: list[pymupdf.Rect | None] = [None] * page_count
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = [
                executor.submit(self._extract_page_rect_from_path, doc_path, i, dpi)
                for i in range(page_count)
            ]
            with tqdm(
                total=page_count,
                desc=f"histogram threaded (dpi={dpi}, workers={max_workers})",
            ) as progress:
                for future in as_completed(futures):
                    page_index, rect = future.result()
                    rectangles[page_index] = rect
                    progress.update(1)

        return [rect for rect in rectangles if rect is not None]

    def _extract_page_rect_from_path(
        self,
        doc_path: str,
        page_index: int,
        dpi: int,
    ) -> tuple[int, pymupdf.Rect]:
        # Use one document instance per worker to avoid cross-thread document access.
        worker_doc = pymupdf.open(doc_path)
        try:
            page = worker_doc.load_page(page_index)
            return (page_index, self._extract_page_rect(page, dpi))
        finally:
            worker_doc.close()

    def _extract_page_rect(
        self,
        page: pymupdf.Page,
        dpi: int | None,
    ) -> pymupdf.Rect:
